...
def pregunta_03():
    ...
    """
    registros = tbl0["_c1"].value_counts()
    registros = registros.sort_index()
    return registros
...
def pregunta_09():
    ...
    """
    pd = tbl0.copy()
    # pd["year"] = pd["_c3"].dt.year
    pd["year"] = pd["_c3"].apply(lambda x: x.split("-")[0])
    return pd
...
def pregunta_11():
    ...
    39   39    a,d,f
    """
    tabla = tbl1.groupby("_c0")["_c4"]
    tabla = tabla.apply(lambda x: ",".join(x))
...
def pregunta_13():
    ...
    suma = tabla.groupby("_c1")["_c5b"].sum()
    return suma
